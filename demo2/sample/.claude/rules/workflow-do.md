...
here, in the auto-loaded rules layer, because `AGENT.md` is only read when an assistant
chooses to open it — an instruction nobody is guaranteed to see is not an instruction.
...
- Brief every agent with: the user story, the frozen contract, and the `.claude/rules/`
  files that apply to its side. An agent starts with none of the conversation's context.
